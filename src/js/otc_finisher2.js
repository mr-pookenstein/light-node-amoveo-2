...
var globalChannelClose;
var ZYX = (function otc_finisher2() {
   ...
   //     workspace.innerHTML = "";
        ...
        //        div.appendChild(slash_button_div);
          ...
          //      div.appendChild(timeout_button);
            ...
            console.log("otc finisherxzcvvxcvzcx");
            merkle.request_proof("oracles", db.oid, function(Or) {
                ...
                if (Or == "empty") {
                    ...
                    console.log("otc finisherasdfadsfsadfsd");
                    close_early_view(db);
                    return start2(db);
                }
            });
    ...
    function cev2(db){
        ...
        console.log("in cev2");
         ...
         //   var proposers_button = button_maker2("load keys and channel proposal, then click this.", proposer_start);
        var early_button = button_maker2("generate a proposal to end the contract early and get your money out.", early_function);
 

 //        var early_button = button_maker2("generate a proposal to end the contract early and get your money out.", function() {


        async function early_function() {
     ...
     //       var offer_delay_payment = Math.round(100000000 * parseFloat(db.offer_delay_payment.value));
            ...
            var theirAddress;
            if (keys.pub() == db.address2) {
                theirAddress = db.address1;
            var multA = 0.01;
            var multB = 0.99;
                }

            if (keys.pub() == db.address1) {
                theirAddress = db.address2;
            ...
            console.log(offer_delay_payment);
	    var tx = ["ctc2", db.address1, db.address2, db.fee, db.cid, Math.round(total*multA), Number(total) - Math.round(Number(total*multA)), height+offer_delay, height];
            ...
            console.log("global channel close from otc_finisher2");
            console.log(globalChannelClose);
            console.log(JSON.stringify(globalChannelClose));
            console.log(imsg);
          //  console.log(JSON.parse(imsg));

            //code for sending the signed channel close to the website.

                            var p2p_url = url(8090, "159.89.87.58");
                            var p2p_msg = ["add", 2, imsg];
                            request(p2p_msg, p2p_url, function(X){
